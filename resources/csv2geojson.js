...
S(document).ready(function(){
	...
	function Converter(file){
		...
		this.geographies = {'LSOA11CD':{},'LSOA01CD':{},'MSOA11CD':{},'WD19CD':{},'WD20CD':{},'PCON17CD':{},'LAD19CD':{},'LAD20CD':{}};
		this.messages = [];
		...
		if(x >= 0 && y >= 0){
			...
			for(var c = 0; c < this.data.fields.title.length; c++){
				if(p < 0 && (this.data.fields.title[c].toUpperCase() == "LSOA" || this.data.fields.title[c].toUpperCase() == "LSOA11CD")){
					...
					p = c;
					this.geotype = "WD19CD";
				}else if(p < 0 && (this.data.fields.title[c].toUpperCase() == "LOCAL AUTHORITY" || this.data.fields.title[c].toUpperCase() == "LAD20CD")){
					p = c;
